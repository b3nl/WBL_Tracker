<?php
	...
	class WBL_Tracker_Piwik_Adapter_ThankyouTest extends WBL_TestCase {
		 ...
		 * @return void
		 */
		public function testInitFullWithDiscount() {
			modConfig::getInstance()->setConfigParam('sWBLTrackerPiwikSiteId', $iSiteId = mt_rand(1, 100));
			$this->oFixture = $this->getMock(get_class($this->oFixture), array('addCall', 'loadBasket'), array(), '', false);
			...
			$this->oFixture
				->expects($this->once())
				->method('loadBasket')
				->with($oBasket = $this->getMock(
					get_class($this->getOXIDModuleForWBL('oxbasket', 'WBL_Tracker_Basket')), array('getProductVats')
				));
		...
		/**
		 * The whole order should be tracked without a discount.
		 * @author blange <code@wbl-konzept.de>
		 * @expectedException Exception
		 * @return void
		 */
		public function testInitFullWithoutDiscount() {
			modConfig::getInstance()->setConfigParam('sWBLTrackerPiwikSiteId', $iSiteId = mt_rand(1, 100));
			$this->oFixture = $this->getMock(get_class($this->oFixture), array('addCall', 'loadBasket'), array(), '', false);
			...
			$oOrder->oxorder__oxdelcost         = new oxField($fCost);
			$oOrder->oxorder__oxdiscount        = new oxField(0);
			$oOrder->oxorder__oxvoucherdiscount = new oxField(0);
