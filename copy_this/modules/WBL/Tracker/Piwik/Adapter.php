    ...
    class WBL_Tracker_Piwik_Adapter extends WBL_Tracker_Adapter_Abstract
    {
                ...
                if ($aItems = $oBasket->getContents()) {
                    ...
                    foreach ($aItems as $oItem) {
                        ...
                        $this->addCall(
                            array(
                                'addEcommerceItem',
                                (string) $oArticle->oxarticles__oxartnum->value ? : $oArticle->getId(),
        ...
        protected function loadECommerceViews(oxUBase $oView = null)
        {
            ...
            if ($oView) {
                ...
                if (($this->withDefaultCat() || $bWithArticle) &&
                    (($oCat = $oView->getActiveCategory()) instanceof oxCategory))
                {
                ...
                if ($bWithArticle && $oProduct = $oView->getProduct()) {
                    $aValues = array(
                        'setEcommerceView',
                        (string) $oProduct->oxarticles__oxartnum->value ? : $oProduct->getId(),
            ...
            foreach ($aConfig as $iIndex => $sRules) {
                ...
                if ($oHelper->checkCondition(trim($sCheck), trim($mView), $sParam = trim($mParam))) {
                    ...
                    if ($sValue = trim($mValue)) {
                        if (($oCall = $oHelper->getCallback(
                                $sValue,
                                $sParam
                            )) instanceof WBL_Tracker_Conditions_Value_Interface
                        ) {
         ...
         * @param  string $sScope The piwik scope.
         * @return WBL_Tracker_Piwik_Adapter
         */
